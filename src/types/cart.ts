// Unified cart system for DXF and Pipe items

import { DxfConfig } from "./dxf";
import { PipeConfig, calculateCutPrice } from "./pipe";
...
export const addTubeItemToCart = (cart: Cart, config: PipeConfig): Cart => {
  const newItem: CartItem = {
...
export const getCartTotalPrice = (cart: Cart): number => {
  return cart.items.reduce((sum, item) => {
    ...
    if (item.type === "tube" && item.pipeConfig) {
      return sum + calculateCutPrice(item.pipeConfig);
    }
    return sum;
  }, 0);
};
